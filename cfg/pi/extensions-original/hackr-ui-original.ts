...
function renderHackrHeader(width: number, model?: string, cwd?: string): string[] {
	...
	const cwdText = cwd ? ` ${fg(C.oyster, ICON.dot)} ${fg(C.oyster, cwd.split("/").slice(-2).join("/"))}` : "";

	const left = ` ${wordmark} `;
	const right = `${modelText}${cwdText}`;
	const leftW = visibleWidth(left);
	const rightW = visibleWidth(right);
	const diagFill = fg(C.charcoal, ICON.diagonal.repeat(Math.max(0, width - leftW - rightW)));

	return [
		truncateToWidth(left + diagFill + right, width),
		fg(C.charcoal, ICON.separator.repeat(width)),
	];
}
...
function renderHackrFooter(
	...
	const branchStr = branch ? ` ${fg(C.charcoal, ICON.dot)} ${fg(C.squid, branch)}` : "";
	const modelName = model ?? "no-model";
	const cwdStr = cwd ? ` ${fg(C.charcoal, ICON.dot)} ${fg(C.oyster, cwd.split("/").slice(-2).join("/"))}` : "";
...
class HackrEditor extends CustomEditor {
	...
	render(width: number): string[] {
		...
		if (this.yoloMode) {
			...
			promptPrefix = yoloBadge + fg(C.bok, bold(":::")) + " " + fg(C.squid, "❯") + " ";
		} else {
		...
		const hintText = this.showExitHint ? fg(C.oyster, "ctrl+c again to exit") : "";
		...
		if (usage?.percent != null && ctxWindow) {
			...
			statsText = fg(ctxColor, fmt(usage.tokens) + "/" + (ctxWindow / 1000).toFixed(0) + "k");
		}
		...
		if (hintText || statsText) {
			...
			lines[lines.length - 1] = truncateToWidth(hintText + " ".repeat(gap) + statsText, width);
		} else {
...
export default function (pi: ExtensionAPI) {
		...
		ui.setFooter((_tui: TUI, _theme: Theme, footerData: any) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					return renderHackrFooter(width, ctx.model?.id, footerData.getGitBranch(), input, output, cost, ctx.cwd);
				},
			};
		});
	...
	pi.registerCommand("yolo", {
		description: "Toggle YOLO mode (auto-accept all permissions + visual indicator)",
		handler: async (_args, ctx) => {
			...
			if (yoloMode) {
				ctx.ui.notify(bg(C.citron, fg(C.pepper, bold(" ! "))) + " YOLO mode ON — all permissions auto-accepted", "warning");
			} else {
	...
	pi.registerCommand("hackr-ui", {
		description: "Toggle Hackr-style UI (header, footer, editor, working indicator)",
		handler: async (_args, ctx) => {
			enabled = !enabled;
			if (enabled) {
				applyHackrUI(ctx);
